...
- aion/68 (follow-up) — Canvas key shortcuts (E/S/T/arrows/Delete) bubble up from any focused descendant, so typing a slot label toggled edit mode or deleted cards. `_handleKeyEvent` now ignores events whose focus is inside an `EditableText`, and clicking the canvas or a card re-focuses the canvas. **Needs visual check:** typing in the slots panel doesn't trigger shortcuts; shortcuts work again after clicking the canvas.
