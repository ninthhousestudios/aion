...
void main() {
  ...
  group('importFile', () {
    ...
    test('imports .toml fixture with correct fields', () {
      final (doc, sourcePath) = importer.importFile(
        '$fixtureDir/test-chart-legacy.toml',
      );

      expect(doc.name, 'Test Person');
      expect(doc.lat, closeTo(40.7128, 0.001));
      expect(doc.lon, closeTo(-74.006, 0.001));
      expect(doc.alt, closeTo(10.0, 0.1));
      expect(doc.gender, 'male');
      expect(doc.placename, 'New York, NY');
      expect(doc.country, 'USA');
      expect(doc.utcOffset, -5.0);
      ...
      expect(doc.jd, closeTo(2451545.0, 0.001));
      expect(sourcePath, contains('test-chart-legacy.toml'));
    });

    test('imports .json fixture with correct fields', () {
      final (doc, _) = importer.importFile('$fixtureDir/test-chart.json');

      expect(doc.name, 'Jane Doe');
      expect(doc.lat, closeTo(51.5074, 0.001));
      expect(doc.lon, closeTo(-0.1278, 0.001));
      expect(doc.alt, 0); // no altitude in JSON format
      expect(doc.gender, 'female');
      expect(doc.placename, 'London');
      expect(doc.country, 'UK');
      expect(doc.utcOffset, 1.0);
      expect(doc.dstOffset, 1.0);
      expect(doc.notes, 'Test chart');
      expect(doc.rodden, 'AA');
      ...
      expect(doc.jd, closeTo(expectedJd, 0.001));
    });
  ...
  group('importDirectory', () {
    ...
    test('imports all charts in fixture directory', () {
      ...
      final names = charts.map((r) => r.$1.name).toSet();
    ...
    test('filters by extension', () {
      ...
      expect(charts.any((r) => r.$1.name == 'Test Person'), isTrue);
    });
    ...
    test('collects errors for bad files', () {
      ...
      expect(charts.any((r) => r.$1.name == 'Jane Doe'), isTrue);
